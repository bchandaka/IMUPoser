...
datadir = "/home/bchan/projects/imu_human/data"
config.raw_amass_path = os.path.join(datadir, "amass")
config.processed_imu_poser = os.path.join(datadir, "processed_imuposer")
def process_single_amass(npz_path):
    def _syn_acc(v):
        r"""
    ...
    data_pose, data_trans, data_beta, length = cdata['poses'][::step].astype(np.float32), cdata['trans'][::step].astype(np.float32), [cdata['betas'][:10]], [cdata['poses'][::step].shape[0]]
    ...
    print('Saving')
    npz_filename = os.path.basename(npz_path).split("_poses.npz")[0]
    npz_dir = os.path.dirname(npz_path)
    amass_dir = npz_dir.replace("/amass/", "/processed_amass/")
    os.makedirs(amass_dir, exist_ok=True)
    ds_dir = os.path.join(amass_dir, npz_filename)
    os.makedirs(ds_dir, exist_ok=True)
    ds_dir = Path(ds_dir).absolute()
...
if __name__ == '__main__':
    ...
    # process_amass()
    npz_path = "/home/bchan/projects/imu_human/data/amass/ACCAD/Female1General_c3d/A6 - lift box_poses.npz"
    process_single_amass(npz_path)
